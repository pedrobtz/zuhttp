...
zu_tls(ca_extra = ca)
#> <zu_tls_config>
#>   ca_extra: /tmp/RtmpyliZhA/zuhttp-tls-200f48572315/ca.pem (adds to system trust)
#>   revocation: off
```
...
#> Error:
#> ! `ca_extra` has no PEM certificate in it: /tmp/RtmpyliZhA/zuhttp-tls-200f48572315/corporate-root-der.crt
#>   It looks like a DER (binary) certificate. zuhttp reads PEM, the text
#>   form starting '-----BEGIN CERTIFICATE-----'. Convert it with:
#>     openssl x509 -inform der -in /tmp/RtmpyliZhA/zuhttp-tls-200f48572315/corporate-root-der.crt -out cert.pem
```
